...
> **位置偏離**：原計畫將 `ProcessRegistry` 放在 `daemon/manager/state.go` 子包，但 Go
> 不允許 `manager` 子包 import `daemon` 同時 `daemon` import `manager`（循環依賴）。
> 最終實作位置為 `daemon/process_registry.go`（同 `daemon` 包內），公開 API 與計畫書
> 完全一致。`ProcessRegistry` 透過 `Server` 結構上的委派方法（`Lock/Unlock/RLock/RUnlock`）
> 對外暴露鎖介面給少數需要跨多次呼叫持鎖的場景（例如 `launchProcess` 的 lookup +
> ID 計數 + map 寫入原子區段）。
...
flowchart TD
    ...
    Manager -->|"調用線程安全接口"| Registry["進程註冊表 (daemon/process_registry.go)"]
...
- `最簡可行解耦設計 (MVE Design)`
  在 `daemon/process_registry.go` 中定義 `ProcessRegistry`，我們可以使用 Go 介面定義以便於單元測試的 mock 注入：
...
- `步驟 1`：建立 `daemon/process_registry.go`。
